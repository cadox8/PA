...
public class SetGroupCMD extends PACmd {
    ...
    public void run (PAUser user, String label, String[] args) {
        ...
        if(args.length == 0){
            user.sendMessage(PAData.PAPlugins.CORE.getPrefix() + "&6Tu Rango: &2" + Grupo.groupColor(user.getUserData().getGrupo()) + user.getUserData().getGrupo().toString());
            return;
        }
        ...
        if (args.length == 1){
            ...
            user.sendMessage(PAData.PAPlugins.CORE.getPrefix() + "&3Rango cambiado &3: " + Grupo.groupColor(Grupo.values()[i]) + Grupo.values()[i].toString());
        }
